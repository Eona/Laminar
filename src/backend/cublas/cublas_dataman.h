 ...
 */
class CublasDataManager :
		public DataManager<CudaFloatMat>
//		public GradientCheckable<>
{
public:
	typedef CudaEngine::CudaFloatMatPtr DataPtr;

	/**
	 * @param randLow lower bound of random input uniform distr
	 * @param randHigh higher bound of random input uniform distr
	 */
	vector<float> dummyData;

	CublasDataManager(vector<float> dummyData) :
		dummyData(dummyData)
	{}
	...
	{
		if (!is_initialized)
			write->reset(inputDim, batchSize);
